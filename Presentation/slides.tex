...
\begin{itemize}
\item Génère $O(p^2)$ permutations possibles
\begin{itemize}
\item $O(p^3)$ si tout les $\alpha_{\pi(j)}$ ne sont pas dévoilés)
\end{itemize}
\item Nécessite certaines conditions (raisonnables) sur $p$ et $h$
\end{itemize}
\end{frame}
