...
where $\alpha_i \in \F$ and $z_i \in \F_q\backslash\{0\}$ for all $i \in \{1,...,n\}$. Note that, if $\alpha_i = \infty$, we replace the $i^t^h$ column by the vector $z_i(0,...,0,1)^T$, so that all the coefficients of the matrix are finite.
...
Then if we consider another generator $g_0$ of $\GF(q)^{\*}$, we have $g_0 = g^u$ and
